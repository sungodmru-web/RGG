...
import { useCreateAdminEndorsement } from "@/hooks/use-admin-api";
...
export default function AdminEndorsementNew() {
  const [, setLocation] = useLocation();
  const { toast } = useToast();
  const createEndorsement = useCreateAdminEndorsement();
  ...
  const onSubmit = (values: EndorsementFormValues) => {
    ...
    createEndorsement.mutate(cleanedValues as any, {
      onSuccess: (data) => {
        toast({ title: "Endorsement created successfully" });
        setLocation(`/admin/endorsements/${data.id}`);
      },
      onError: (error) => {
  ...
  return (
    <div className="container mx-auto p-6 max-w-4xl space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-4 text-sm text-[#9AA79F] mb-2">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-serif text-[#F4F1EA]">New Endorsement</h1>
          <p className="text-[#9AA79F] mt-1">Add a new testimonial or endorsement quote.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            disabled={createEndorsement.isPending || isUploading}
